...
describe('路由 => 【用户】测试套件', function () {
  it('正确的用户名和密码登录操作', function (done) {
    request(app.listen())
      ...
      .send(trueUser)
      .expect('set-cookie', 'uid=1506766620306; path=/; secure; httponly,login=bingo; path=/; secure; httponly')
      .expect(200, done)
  })
  it('正确的用户名密码和记住我登录操作', function (done) {
    request(app.listen())
      ...
      .expect('set-cookie', /uid=1506766620306; path=\/; expires=\w{3},\s\d\d\s\w{3}\s\d\d\d\d\s\d\d:\d\d:\d\d\sGMT; secure; httponly,login=bingo; path=\/; expires=\w{3},\s\d\d\s\w{3}\s\d\d\d\d\s\d\d:\d\d:\d\d\sGMT; secure; httponly/)
      .expect(200, done)
  })
