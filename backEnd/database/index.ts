 ...
 * */
const getMomentsList = async (condition: DatabaseInterfaces.momentsQuery) => {
    let skip = 0;
    const query: any = {};
    ...
    if (condition.date) {
        query.date = condition.date
    }
    if (condition.dateStr) {
        query.dateStr = condition.dateStr
    }
    return await momentsModel.find(query, "-_id -__v").sort({date: -1}).skip(skip).limit(condition.limit || 0)
};
 ...
 * */
const saveMoments = async (moments: DatabaseInterfaces.moments) => {
 ...
 * */
const updateMoments = async (moments: DatabaseInterfaces.updateMoments) => {
...
const deleteMoments = async (date: number) => {
    ...
    try {
        ...
        throw new Error("Invalid delete argument." + e)
    }
};
...
const buildBlogSummary = async () => {
    ...
    allBlog.forEach((blog: DatabaseInterfaces.blogDocument) => {
